...
stargazer(fe.ov[fe.ov$YEAR >= 1988,c(22,15:18,5:14, 19:21, 24:26)], type="latex", style="asr", title="Unweighted Descriptive Statistics",
          summary=T, median=T, covariate.labels=c("Felony History Pct.","Not Employed Rate","Unemployment Rate", 
...
```{r, include=F}
corr<-round(cor(fe.ov[fe.ov$YEAR >= 1988,-c(1:14,27,28)], use="pairwise.complete.obs", method="pearson" ),2)
corr[upper.tri(corr)] <-""
upper <- as.matrix(corr)
rownames(upper) <- c("Not Employed Rate","Unemployment Rate", "Idleness Rate", "Can't Find Work Rate",
                                                  "Disability Rate","SSI Rate","Marriage Rate",
                                                  "Felony History Pct.","SSDI Rate",
                                                  "Effective Wage","Mean TANF Maximum",
                                                  "Unemployment Comp.")
...
fe.fd <- fe.ov %>% filter( YEAR >= 1988) %>%
  select(YEAR, STATENAME, p6.y1.notemployed.rate, p6.y3.idle.rate, 
                      p6.y2.unemployed.rate, p6.y4.cfw.rate, pctexfel, time.indicator) %>% 
               group_by(STATENAME) %>%
...
```{r, echo=F, warning=F}
ggplot(fe.fd, aes(x=pctexfel.fd, y=p6.y1.fd)) +
...
ggplot(fe.fd, aes(x=pctexfel.fd, y=p6.y2.fd, colour=time.indicator)) +
  geom_point()+
  geom_smooth(method="lm",se=F, colour="red")+
  theme_bw()+
  xlab("1-Year Change in Percent Ex Felon")+
...
```{r, echo=F, warning=F, include=F}
ggplot(fe.fd, aes(x=pctexfel.fd, y=p6.y3.fd, colour=time.indicator)) +
  geom_point()+
  geom_smooth(method="lm",se=F, colour="red")+
  theme_bw()+
  xlab("1-Year Change in Percent Ex Felon")+
...
```{r, echo=F, warning=F, include=F}
ggplot(fe.fd[fe.fd$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),], aes(x=pctexfel.fd, y=p6.y4.fd, colour=time.indicator)) +
  geom_point()+
  geom_smooth(method="lm",se=F, colour="red")+
  theme_bw()+
  xlab("1-Year Change in Percent Ex Felon")+
...
y1.fe <- plm(p6.y1.notemployed.rate~pctexfel, data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight, 
...
y2.fe <- plm(p6.y2.unemployed.rate~pctexfel, data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight,
             index = c("STATENAME","YEAR"), effect="twoway", model="within")
y2.fe.robust <- coeftest(y2.fe, vcov=vcovHC(y2.fe,type="HC0",cluster=c("group","time")))

y2.fe.2 <- plm(p6.y2.unemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus, 
               data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight, index = c("STATENAME","YEAR"), effect="twoway", model="within")
y2.fe.2.robust <- coeftest(y2.fe.2, vcov=vcovHC(y2.fe.2,type="HC0",cluster=c("group","time")))


y2.fe.3 <- plm(p6.y2.unemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                 t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate, data = fe.ov[fe.ov$YEAR >= 1988,], 
               weights = norm.weight,
               index = c("STATENAME","YEAR"), effect="twoway", model="within")
y2.fe.3.robust <- coeftest(y2.fe.3, vcov=vcovHC(y2.fe.3,type="HC0",cluster=c("group","time")))


y2.fe.4 <- plm(p6.y2.unemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
...
y3.fe <- plm(p6.y3.idle.rate~pctexfel, data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight,
             index = c("STATENAME","YEAR"), effect="twoway", model="within")
y3.fe.robust <- coeftest(y3.fe, vcov=vcovHC(y3.fe,type="HC0",cluster=c("group","time")))

y3.fe.2 <- plm(p6.y3.idle.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus, 
               data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight, index = c("STATENAME","YEAR"), effect="twoway", model="within")
y3.fe.2.robust <- coeftest(y3.fe.2, vcov=vcovHC(y3.fe.2,type="HC0",cluster=c("group","time")))


y3.fe.3 <- plm(p6.y3.idle.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate,
...
y3.fe.4 <- plm(p6.y3.idle.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                 t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p6.disab.rate+p6.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
                  data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight, index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
```{r, results='asis', echo=F, include=F}
stargazer(y3.fe, y3.fe.2, y3.fe.3, y3.fe.4,
          type="latex", style="aer", title="Fixed Effects Regression", covariate.labels= c(
                                                  "Felony History Pct.", 
                                                  "Pop. Share 26-35",
                                                  "Pop. Share 36-45","Pop. Share 46-55",
...
y4.fe <- plm(p6.y4.cfw.rate~1+pctexfel, data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight,
             index = c("STATENAME","YEAR"), effect="twoway", model="within")
y4.fe.robust <- coeftest(y4.fe, vcov=vcovHC(y4.fe,type="HC0",cluster=c("group","time")))

y4.fe.2 <- plm(p6.y4.cfw.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus, 
               data = fe.ov[fe.ov$YEAR >= 1988,], weights = norm.weight, index = c("STATENAME","YEAR"), effect="twoway", model="within")
y4.fe.2.robust <- coeftest(y4.fe.2, vcov=vcovHC(y4.fe.2,type="HC0",cluster=c("group","time")))


y4.fe.3 <- plm(p6.y4.cfw.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
               t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate, 
               data = fe.ov[fe.ov$YEAR >= 1988,], 
               weights = norm.weight,
               index = c("STATENAME","YEAR"), effect="twoway", model="within")
...
y4.fe.4 <- plm(p6.y4.cfw.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                 t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p6.disab.rate+p6.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
                  data = fe.ov[fe.ov$YEAR >= 1988,], 
                  weights = norm.weight, 
...
y4.fe.robust.se <- list(y4.fe.robust[,2], y4.fe.2.robust[,2], y4.fe.3.robust[,2], y4.fe.4.robust[,2])
y4.fe.robust.p <- list(y4.fe.robust[,4], y4.fe.2.robust[,4], y4.fe.3.robust[,4], y4.fe.4.robust[,4])

alt.fe.robust.se <- list(y3.fe.4.robust[,2],y2.fe.4.robust[,2], y4.fe.4.robust[,2])
alt.fe.robust.p <- list(y3.fe.4.robust[,4],y2.fe.4.robust[,4], y4.fe.4.robust[,4])

```


```{r, results='asis', echo=F, include=F}
stargazer(y4.fe, y4.fe.2, y4.fe.3, y4.fe.4, 
          type="latex", style="aer", title="Fixed Effects Regression", covariate.labels= c(
                                                  "Felony History Pct.", 
                                                  "Pop. Share 26-35",
                                                  "Pop. Share 36-45","Pop. Share 46-55",
...
```{r, results='asis', echo=F}
stargazer(y3.fe.4, y2.fe.4, y4.fe.4,
...
p6.y1.male <- plm(p6.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                  ...
                  data = fe.p6.m, weights = norm.weight, index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
p6.y1.female <- plm(p6.y1.notemployed.rate~pctexfel+
                  popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p6.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
                  data = fe.p6.female, weights = norm.weight, index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
fe.p6.black <- fe %>% select(YEAR, STATENAME, STATEFIP, p6.pop.b, popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p6.marriage.rate.b, p6.disab.rate.b, pctexfel, ssdi.rate, effective.wage, 
                       TANF.mu, z_labor_unemployment_compensatio,time.indicator) %>%
                      mutate(ssi.rate=ssi.rate, norm.weight = p6.pop.b*(1550/sum(p6.pop.b, na.rm=T))) %>% rename(marriage.rate=p6.marriage.rate.b, p6.disab.rate = p6.disab.rate.b)
...
p6.y1.black <- plm(p6.y1.notemployed.rate.b~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                  ...
                  data = fe.p6.black, weight = norm.weight, index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
p6.y1.white <- plm(p6.y1.notemployed.rate.w~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                  t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p6.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
                  data = fe.p6.white, weights = norm.weight, index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
fe.p2.all <- fe %>% select(YEAR, STATENAME, STATEFIP, p2all.pop, popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p2.marriage.rate.all, p2.disab.rate, pctexfel, ssdi.rate, effective.wage, 
                       TANF.mu, z_labor_unemployment_compensatio,time.indicator) %>%
                      mutate(ssi.rate=ssi.rate, 
                             norm.weight = p2all.pop*(1550/sum(p2all.pop)),marriage.rate=p2.marriage.rate.all, p6.disab.rate = p2.disab.rate)

p2.y1.work <- plm(p2.y1.notemployed.rate.all~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                   p6.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
...
alt.y1.fe.robust.se <- list(p6.y1.male.robust[,2], p6.y1.female.robust[,2], p6.y1.black.robust[,2], p6.y1.white.robust[,2], p2.y1.work.robust[,2])
alt.y1.fe.robust.p <- list(p6.y1.male.robust[,4], p6.y1.female.robust[,4], p6.y1.black.robust[,4], p6.y1.white.robust[,4], p2.y1.work.robust[,4])
```

```{r, results='asis', echo=F}
stargazer(p6.y1.male, p6.y1.female, p6.y1.black, p6.y1.white, p2.y1.work,
          ...
          column.labels = c("Male","Female","Black","White","WA"), header=F, p.auto=F,
          notes=c("Clustered Standard Errors by State and Year.", "Weighted by State-Year total pop."),
...
m <- lm(p6.y1.notemployed.rate~pctexfel+as.factor(STATENAME)+as.factor(YEAR)+
                     +popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                       p6.disab.rate+p6.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+
                        t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate,
...
ggplot(bt, aes(x=start.year, y=est)) +
  ...
  ylab("Estimate")+
  ggtitle("Felony History Pct. FE Coefficient Over Time")
...
ggplot(bt, aes(x=start.year, y=est)) +
  ...
  xlab("5-Year Window Center")+
  ylab("Estimate")+
  ggtitle("Male Felony History Pct. FE Coefficient Over Time")
...
ggplot(bt, aes(x=start.year, y=est)) +
  ...
  xlab("5-Year Window Center")+
  ylab("Estimate")+
  ggtitle("Female Felony History Pct. FE Coefficient Over Time")
...
y1.fe.d <- plm(p6.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                  t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p6.disab.rate+ssi.rate+p6.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio,
...
# Time Series Plots
```{r, echo=F, warnings=F}
#chris crim deportation paper analysis
#scatterplot of first differences at the year level

#sarah give numerators, divide by tailored pops, plot first differences against each other, 
  #follow crim deportation paper, both in percentage point change

library(readxl)
library(tidyr)

fe.ts <- fe.p6.female %>% select(STATENAME, YEAR, pctexfel, p6.y1.notemployed.rate, p6.pop.female) %>% group_by(YEAR) %>%
              summarise(notemployed = mean(p6.y1.notemployed.rate, na.rm=T), pctexfel = mean(pctexfel, na.rm=T),
                        fem.pop = sum(p6.pop.female, na.rm=T))


nat <- read_excel(path = "Shannon_etal_2017_National_Estimates_1948-2010.xlsx") %>% slice(1:63) %>% 
  select(Year, `Total Number of Males with a Felony Record (ex- and current)`, `Total Number with a Felony Record (ex- and current)`) %>%
  rename(total.male.record = `Total Number of Males with a Felony Record (ex- and current)`,
         total.record = `Total Number with a Felony Record (ex- and current)`) %>%
  mutate(total.female.record = total.record-total.male.record, ratio = total.male.record/total.female.record,
         Year = as.integer(Year))


fe.ts <- fe.ts %>% left_join(nat, by = c("YEAR" = "Year")) %>% 
  mutate(fexfel = total.female.record/fem.pop, fexfel.fd =  fexfel - dplyr::lag(fexfel),
         femp.fd = notemployed - dplyr::lag(notemployed))

fe.ts.plot <- fe.ts %>% select(YEAR, fexfel.fd, femp.fd) %>% gather(key = "variable", value = "value", -YEAR)

ggplot(fe.ts.plot, aes(x=YEAR, y=value))+
...
ggplot(usa) +
  ...
  scale_fill_distiller(palette = "Spectral",
                       ...
                       limits = c(1,8),
                      name = "Percent") +
  ggtitle("Felony History Pct. in the United States: 1980-2010",
...
ggplot(usa.2010) +
  ...
  scale_fill_distiller(palette = "Spectral",
                       ...
                       limits = c(0,13),
                      name = "Percent") +
  ggtitle("Felony History Pct. in the United States: 2010",
          subtitle = "Shannon et al. 2017")+
  theme(axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
