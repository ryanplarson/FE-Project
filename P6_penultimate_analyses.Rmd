...
#impute in black subsample model & include missing dummy
#rolling window plots for male and female models
...
```{r, include=F}
corr<-round(cor(fe.ov[,-c(1:14,27,28)], use="pairwise.complete.obs", method="pearson" ),2)
corr[upper.tri(corr)] <-""
...
colSums(is.na(fe.p6.black))
fe.p6.black[is.na(fe.p6.black$p6.y1.notemployed.rate.b),c("YEAR","STATENAME")]
fe.p6.black[is.na(fe.p6.black$marriage.rate),c("YEAR","STATENAME")]
...
stargazer(p6.y1.male, p6.y1.female, p6.y1.black, p6.y1.white, p2.y1.work,
          type="latex", style="aer", title="Alternative Sample Models" , covariate.labels= c(
                                                  ...
                                                  "Ovr. Unemp. Rate t-1",
                                                  "Ovr. Unemp. Rate t-2","Ovr. Unemp. Rate t-3"),
          dep.var.labels=c("Male","Female","Black","White","WA"), header=F, p.auto=F,
          notes=c("Clustered Standard Errors by State and Year.", "Weighted by State-Year total pop."),
...
ggplot(bt, aes(x=start.year, y=est)) +
  ...
  ylab("Estimate")+
  ggtitle("Felony History Pct. FE Coefficient Over Time - Rolling Window Estimates")
