...
fe.ts <- fe.p6.female %>%  group_by(YEAR) %>%
...
fe.ts <- fe.ts %>% left_join(nat, by = c("YEAR" = "Year")) %>% 
  mutate(fexfel = 100*(total.female.record/fem.pop), fexfel.fd =  (fexfel - dplyr::lag(fexfel))/dplyr::lag(fexfel),
         femp.fd = (notemployed - dplyr::lag(notemployed))/dplyr::lag(notemployed))
...
ggplot(fe.ts.plot, aes(x=YEAR, y=value))+
  ...
  ggtitle("Female Records and Non-Employment", 
          subtitle = "Percentage Changes")+
  labs(x = "Year", y ="Percentage Change")+
  scale_x_continuous(breaks = c(seq(1982,2010, 4)))+
  theme_minimal()
```

#Preliminary TS Analyses

```{r, warning=F, message=F}
# Time Series Analyses
library(astsa)
library(tseries)
fe.ts.2 <- as.ts(fe.ts$notemployed, start = c(1980,1))

#Autocorrelation? 
  #Durbin-Watson Test - checks for AR(1) in residuals 
m <- lm(notemployed~fexfel, data=fe.ts)

plot(m)

acf2(resid(m)) #residuals appear AR(1)
dwtest(m, alternative = "two.sided") #durbin-wqatson

#Stationarity? DF unit root test
adf.test(fe.ts.2, k=0) #standard DF test

#high correlation with year
with(as.data.frame(fe.ts), cor(fexfel, YEAR, method = "pearson"))


#preliminary model
fe.ts.2 <- as.ts(fe.ts$notemployed, start = 1980, end = 2010)
sarima(fe.ts.2, 1,1,0, xreg = fe.ts$fexfel)
...
fe.ov.7 <- fe %>% select(YEAR, STATENAME, STATEFIP, p7.pop,popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p7.marriage.rate, pctexfel, ssdi.rate, effective.wage, 
...
p7.y1 <- plm(p7.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                 t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                 p7.disab.rate+p7.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+p7.degree.rate,
...
fe.p7.m <- fe %>% select(YEAR, STATENAME, STATEFIP, p7.pop,popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p7.marriage.rate.m, pctexfel, ssdi.rate, effective.wage, 
...
p7.y1.male <- plm(p7.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                   p7.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+p7.degree.rate,
                  data = fe.ov.7.m[fe.ov.7.m$YEAR >= 1988,], weights = norm.weight, 
                  index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
fe.p7.female <- fe %>% select(YEAR, STATENAME, STATEFIP, p7.pop.female, popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p7.marriage.rate.female, p7.disab.rate.female, pctexfel, ssdi.rate, effective.wage, 
                       TANF.mu, z_labor_unemployment_compensatio,p7.degree.rate,time.indicator) %>%
                      ...
                      rename(marriage.rate= p7.marriage.rate.female, p7.disab.rate = p7.disab.rate.female,
                             ...
                             p7.degree.rate = p7.degree.rate.f)

p7.y1.female <- plm(p7.y1.notemployed.rate~pctexfel+
                  popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p7.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+p7.degree.rate,
                  data = fe.ov.7.female[fe.ov.7.female$YEAR >= 1988,], weights = norm.weight, 
                  index = c("STATENAME","YEAR"), model="within",effect="twoway")
...
fe.ov.8 <- fe %>% select(YEAR, STATENAME, STATEFIP, p8.pop,popshare.16.25, popshare.26.35, popshare.36.45,
                       popshare.46.55, popshare.56.65, popshare.66.plus, t.unemp.rate,
                       t_1.unemp.rate, t_2.unemp.rate, t_3.unemp.rate, p8.y1.notemployed.rate,
                       p8.y2.unemployed.rate, p8.y3.idle.rate, p8.y4.cfw.rate,p8.disab.rate, ssi.rate,
                       p8.marriage.rate, pctexfel, ssdi.rate, effective.wage, 
...
p8.y1 <- plm(p8.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                 t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                 p8.disab.rate+p8.marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+p8.degree.rate,
                  data = fe.ov.8[fe.ov.8$YEAR >= 1988,], weights = norm.weight, 
             index = c("STATENAME","YEAR"), model="within",effect="twoway")
p8.y1.robust <- coeftest(y1.fe.4, vcov=vcovHC(y1.fe.4,type="HC0",cluster=c("group","time")))
...
fe.p8.m <- fe %>% select(YEAR, STATENAME, STATEFIP, p8.pop,popshare.16.25, popshare.26.35, popshare.36.45,
                       ...
                       p8.y2.unemployed.rate.m, p8.y3.idle.rate.m, p8.y4.cfw.rate.m,p8.disab.rate.m, ssi.rate,
                       p8.marriage.rate.m, pctexfel, ssdi.rate, effective.wage, 
...
p8.y1.male <- plm(p8.y1.notemployed.rate~pctexfel+popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
...
fe.p8.female <- fe %>% select(YEAR, STATENAME, STATEFIP, p8.pop.female, popshare.16.25, popshare.26.35, popshare.36.45,
                       popshare.46.55, popshare.56.65, popshare.66.plus, t.unemp.rate,
                       t_1.unemp.rate, t_2.unemp.rate, t_3.unemp.rate, p8.y1.notemployed.rate.female,
                       p8.y2.unemployed.rate.female, p8.y3.idle.rate.female, p8.y4.cfw.rate.female, ssi.rate,
                       p8.marriage.rate.female, p8.disab.rate.female, pctexfel, ssdi.rate, effective.wage, 
                      ...
                      rename(marriage.rate= p8.marriage.rate.female, p8.disab.rate = p8.disab.rate.female,
                             p8.y1.notemployed.rate = p8.y1.notemployed.rate.female,
                             p8.degree.rate = p8.degree.rate.female)

p8.y1.female <- plm(p8.y1.notemployed.rate~pctexfel+
                  popshare.26.35+popshare.36.45+popshare.46.55+popshare.56.65+popshare.66.plus+
                    t.unemp.rate+t_1.unemp.rate+t_2.unemp.rate+t_3.unemp.rate+
                  p8.disab.rate+marriage.rate+effective.wage+TANF.mu+z_labor_unemployment_compensatio+p8.degree.rate,
