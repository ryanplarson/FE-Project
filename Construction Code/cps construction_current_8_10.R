...
cps.emp <- cps %>% select(YEAR, STATENAME, WTSUPP) %>%
  group_by(YEAR, STATENAME) %>% summarize(population.16=sum(WTSUPP, na.rm=T))


#population variable - share of 16+ popularion overall
cps.popshare <- cps  %>% select(YEAR, STATENAME, pop.share, WTSUPP) %>% 
  group_by(YEAR, STATENAME, pop.share) %>% summarize(pop=sum(WTSUPP, na.rm=T)) %>%
...
cps.unemp <- cps %>% select(YEAR, STATENAME, employed, WTSUPP) %>% 
  group_by(YEAR, STATENAME, employed) %>% summarize(overall=sum(WTSUPP, na.rm=T)) %>%
...
#p1
cps.p1 <- cps %>% select(YEAR, STATENAME, degree, prime, gender,WTSUPP) %>% 
...
#p1.y1
cps.p1.y1 <- cps %>% select(YEAR, STATENAME, degree,employed.share, prime, gender, WTSUPP) %>% 
...
#p1.y2 
cps.p1.y2 <- cps %>% select(YEAR, STATENAME, degree, employed, prime, gender, WTSUPP) %>% 
...
# 1995 Nebraska?? weights sum to zero for p1 - entered 0 for now
sum(cps[cps$YEAR==1995 & cps$STATENAME=="Nebraska" & cps$employed=="Unemployed" & cps$gender=="Male" & cps$degree=="No" & cps$prime=="Prime",]$WTSUPP, na.rm=T)
cps.p1.y2[cps.p1.y2$YEAR==1995 & cps.p1.y2$STATENAME=="Nebraska",]$p1.y2.raw <- 0
cps.p1.y2[cps.p1.y2$YEAR==1995 & cps.p1.y2$STATENAME=="Nebraska",]$p1.y2.unemployed.rate <- 0
...
#p1.y3
cps.p1.y3 <- cps %>% select(YEAR, STATENAME, degree,idle, prime, gender, WTSUPP) %>% 
...
#p1.y4
cps.p1.y4 <- cps %>% select(YEAR, STATENAME, degree,cfw, prime, gender, DWWT) %>% 
...
# Fill in Missing Values (sum to zero) - talk with Chris and everybody about this
cps.p1.y4[is.na(cps.p1.y4$p1.y4.raw) & cps.p1.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p1.y4.raw <- 0
...
# Fill in Missing Values (sum to zero)
cps.p1.y4[is.na(cps.p1.y4$p1.y4.raw.female) & cps.p1.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p1.y4.raw.female <- 0
...
cps.p1.dis <- cps %>% select(YEAR, STATENAME, degree,disabin, prime, gender, WTSUPP) %>% 
...
cps.emp <- left_join(cps.emp, cps.p1.dis, by = c("YEAR","STATENAME")) %>%
...
cps.p1.marry <- cps %>% select(YEAR, STATENAME, degree,marry, prime, gender, WTSUPP) %>% 
...
cps.p2all <- cps %>% select(YEAR, STATENAME, work.age,WTSUPP) %>% 
  group_by(YEAR, STATENAME, work.age) %>% summarize(overall=sum(WTSUPP, na.rm=T)) %>% 
...
#p2.y1.all
cps.p2.y1.all <- cps %>% select(YEAR, STATENAME, employed.share, work.age, WTSUPP) %>% 
  group_by(YEAR, STATENAME, employed.share, work.age) %>% summarize(overall=sum(WTSUPP, na.rm=T)) %>% 
...
#p2.marry.all
cps.p2.marry.all <- cps %>% select(YEAR, STATENAME, marry, work.age, WTSUPP) %>% 
  group_by(YEAR, STATENAME, marry,  work.age) %>% summarize(overall=sum(WTSUPP, na.rm=T)) %>% 
...
#p2
cps.p2 <- cps %>% select(YEAR, STATENAME, degree, work.age, gender,WTSUPP) %>% 
...
#p2.y1
cps.p2.y1 <- cps %>% select(YEAR, STATENAME, degree,employed.share, work.age, gender, WTSUPP) %>% 
...
#p2.y2 
cps.p2.y2 <- cps %>% select(YEAR, STATENAME, degree, employed, work.age, gender, WTSUPP) %>% 
...
#p2.y3
cps.p2.y3 <- cps %>% select(YEAR, STATENAME, degree,idle, work.age, gender, WTSUPP) %>% 
...
#p2.y4
cps.p2.y4 <- cps %>% select(YEAR, STATENAME, degree,cfw, work.age, gender, DWWT) %>% 
...
# Fill in Missing Values (sum to zero)
cps.p2.y4[is.na(cps.p2.y4$p2.y4.raw) & cps.p2.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p2.y4.raw <- 0
...
# Fill in Missing Values (sum to zero)
cps.p2.y4[is.na(cps.p2.y4$p2.y4.raw.female) & cps.p2.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p2.y4.raw.female <- 0
...
#diability
cps.p2.dis <- cps %>% select(YEAR, STATENAME, degree,disabin, work.age, gender, WTSUPP) %>% 
...
cps.emp <- left_join(cps.emp, cps.p2.dis, by = c("YEAR","STATENAME")) %>%
  mutate(p2.disab.rate = 100*(p2.disab.raw/p2.pop))


#marriage and cohabitation rates
cps.p2.marry <- cps %>% select(YEAR, STATENAME, degree,marry, work.age, gender, WTSUPP) %>% 
...
cps.p2.marry <- cps %>% select(YEAR, STATENAME, degree,marry, work.age, gender, WTSUPP) %>% 
  group_by(YEAR, STATENAME, marry, degree, work.age, gender) %>% summarize(overall=sum(WTSUPP, na.rm=T)) %>% 
  unite(degree_age, degree, work.age) %>%  unite(degree_age_gen, degree_age, gender) %>% unite(degree_age_gen_marry, degree_age_gen, marry) %>%
  spread(degree_age_gen_marry, overall) %>% select(YEAR, STATENAME, 'No_Working Age_Female_Yes') %>% rename(p2.marry.raw.female = 'No_Working Age_Male_Yes') 
...
#p3
cps.p3 <- cps %>% select(YEAR, STATENAME, prime, gender,WTSUPP) %>% 
...
#p3.y1
cps.p3.y1 <- cps %>% select(YEAR, STATENAME, employed.share, prime, gender, WTSUPP) %>% 
...
#p3.y2 
cps.p3.y2 <- cps %>% select(YEAR, STATENAME, employed, prime, gender, WTSUPP) %>% 
...
# 1995 Nebraska
sum(cps[cps$YEAR==1995 & cps$STATENAME=="Nebraska" & cps$employed=="Unemployed" & cps$gender=="Male" &  cps$prime=="Prime",]$WTSUPP, na.rm=T)
cps.p3.y2[cps.p3.y2$YEAR==1995 & cps.p3.y2$STATENAME=="Nebraska",]$p3.y2.raw <- 0
cps.p3.y2[cps.p3.y2$YEAR==1995 & cps.p3.y2$STATENAME=="Nebraska",]$p3.y2.unemployed.rate <- 0
...
#p3.y3
cps.p3.y3 <- cps %>% select(YEAR, STATENAME, idle, prime, gender, WTSUPP) %>% 
...
#p3.y4
cps.p3.y4 <- cps %>% select(YEAR, STATENAME, cfw, prime, gender, DWWT) %>% 
...
cps.p3.y4[is.na(cps.p3.y4$p3.y4.raw) & cps.p3.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p3.y4.raw <- 0
...
cps.p3.y4[is.na(cps.p3.y4$p3.y4.raw.female) & cps.p3.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p3.y4.raw.female <- 0
...
#diability
cps.p3.dis <- cps %>% select(YEAR, STATENAME, disabin, prime, gender, WTSUPP) %>% 
...
#disability rate - prime age males without BA
cps.emp <- left_join(cps.emp, cps.p3.dis, by = c("YEAR","STATENAME")) %>%
  mutate(p3.disab.rate = 100*(p3.disab.raw/p3.pop))


#marriage and cohabitation rates
cps.p3.marry <- cps %>% select(YEAR, STATENAME, marry, prime, gender, WTSUPP) %>% 
...
#p4
cps.p4 <- cps %>% select(YEAR, STATENAME, prime,WTSUPP) %>% 
...
#p4.y1
cps.p4.y1 <- cps %>% select(YEAR, STATENAME, employed.share, prime, WTSUPP) %>% 
...
#p4.y2 
cps.p4.y2 <- cps %>% select(YEAR, STATENAME, employed, prime, WTSUPP) %>% 
...
#p4.y3
cps.p4.y3 <- cps %>% select(YEAR, STATENAME, idle, prime, WTSUPP) %>% 
...
#p4.y4
cps.p4.y4 <- cps %>% select(YEAR, STATENAME, cfw, prime, DWWT) %>% 
...
cps.p4.y4[is.na(cps.p4.y4$p4.y4.raw) & cps.p4.y4$YEAR %in% c(1996,1998,2000,2002,2004,2006,2008,2010),]$p4.y4.raw <- 0
...
#diability
cps.p4.dis <- cps %>% select(YEAR, STATENAME, disabin, prime, WTSUPP) %>% 
...
#disability rate - prime age males without BA
cps.emp <- left_join(cps.emp, cps.p4.dis, by = c("YEAR","STATENAME")) %>%
  mutate(p4.disab.rate = 100*(p4.disab.raw/p4.pop))


#marriage and cohabitation rates
cps.p4.marry <- cps %>% select(YEAR, STATENAME, marry, prime, WTSUPP) %>% 
